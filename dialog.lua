  ...
  new = function(phrases, callback)
    ...
    function self.draw(d1, d2, d3, d4)
      if self.current > 0 then
        ...
        love.graphics.rectangle("fill", phrases[self.current].body:getX() - 23, phrases[self.current].body:getY() - 90, 14*#phrases[self.current].text, 31)
        love.graphics.setColor(0, 0, 0, 1)
        love.graphics.print(phrases[self.current].text, phrases[self.current].body:getX() - 23, 
          phrases[self.current].body:getY() - 90, 0, 2, 2)
