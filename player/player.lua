  ...
  new = function(world, playerData, isRemote)
    ...
    function self:mousepressed(xMousepressed, yMousepressed, b)
      if self.inventoryIsOpen then -- проверяем, открыт ли инвентарь
        local status, callback = pcall(function() return self.inventoryGui:mousepressed(xMousepressed, yMousepressed, b, self.activeBox) or self.activeBox.inventoryGui:mousepressed(xMousepressed, yMousepressed, b, self.inventory) or false end)
    ...
    function self:openBox(box)
      box.inventoryIsOpen = not box.inventoryIsOpen
      self.inventoryIsOpen = not self.inventoryIsOpen
