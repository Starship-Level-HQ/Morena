...
noobhub = {
    ...
    new = function(params) -- constructor method
        ...
        function self:subscribe(params)
            ...
            -- Получаем IP и порт, который был назначен сокету
        ...
        function self:publish(message)
            ...
            local send_result, message, num_bytes = self.sock:send("__JSON__START__" ..
                json.encode(message.message) .. "__JSON__END__")
        ...
        function self:enterFrame()
            ...
            for i, v in ipairs(input) do                               -------------
                ...
                while true do
                    ...
                    if (skt) then
                        self.buffer = self.buffer .. skt; got_something_new = true;
                    end
                    if (p) then
                        self.buffer = self.buffer .. p; got_something_new = true;
                    end
                ...
                while got_something_new do
                    local start = string.find(self.buffer, '__CLIENT_LIST__')
                    local finish = string.find(self.buffer, '__CLIENT_LIST_END__')
                    if (start and finish) then -- found a message with the client list
                        local clientListJson = string.sub(self.buffer, start + 15, finish - 1)
                        self.buffer = string.sub(self.buffer, 1, start - 1) ..
                            string.sub(self.buffer, finish + 19) -- cutting our message from buffer
                        local newClientList = json.decode(clientListJson)
                        -- Обновляем глобальный clientList
