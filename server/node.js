...
function getClientList(channel) {
    const channelSockets = sockets[channel];
    if (!channelSockets) return [];
    return Object.keys(channelSockets);
}
...
server.on('connection', (socket) => {
    ...
    socket.on('data', (dataRaw) => {
        ...
        if (
            ...
            // Формируем список клиентов и отправляем его всем подписчикам
            const clientList = getClientList(socket.channel);
            const clientListPayload = '__CLIENT_LIST__' + JSON.stringify(clientList) + '__CLIENT_LIST_END__';

            const channelSockets = sockets[socket.channel];
            const subscribers = Object.values(channelSockets);
            for (let sub of subscribers) {
                sub.isConnected && sub.write(clientListPayload);
            }
        }
